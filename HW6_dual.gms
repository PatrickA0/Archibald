...
VARIABLES X(crops) acres planted
          ...
          Y(resources) value per resource used
...
EQUATIONS
   ...
   Res_Constrain_Dual(crops) profit;

PROFIT..                       VPROFIT =E= SUM(crops,c(crops)*X(crops));
RES_CONSTRAIN(resources) ..    SUM(crops,A(crops,resources)*X(crops)) =L= b(resources);
Cost_Dual..                    VREDCOST =E= SUM(resources, b(resources)*Y(resources));
Res_Constrain_Dual(crops)..    SUM(resources,A(crops,resources)*Y(resources)) =G= c(crops);
...
*DUAL portion of the model
MODEL PLANTING_DUAL /Cost_Dual, Res_Constrain_Dual/;
